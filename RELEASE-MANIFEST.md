...
| **Offline test suite** | **191 / 191** passing (control-plane + **28 CDK stack-synthesis** assertions) |
...
- **191 offline tests** — the CI suite (control-plane + 28 CDK synthesis + 3 CI-completeness gates). Authoritative offline number. Locally you see `127 passed, 1 skipped`: one gate asserts the CDK libs are installed and runs only inside CI.
