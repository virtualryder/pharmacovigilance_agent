...
| **Offline test suite** | **224 / 218** passing on current main (control-plane + **41 CDK stack-synthesis** assertions); 1 CI-completeness gate runs only in CI |
...
- **224 offline tests** — the CI suite (control-plane + 41 CDK synthesis + 3 CI-completeness gates). Authoritative offline number. Locally you see `223 passed, 1 skipped`: one gate asserts the CDK libs are installed and runs only inside CI.
