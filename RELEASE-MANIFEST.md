...
| **Offline test suite** | **144 / 144** passing (control-plane + **23 CDK stack-synthesis** assertions) |
...
- **144 offline tests** — the CI suite (control-plane + 23 CDK synthesis + 3 CI-completeness gates). Authoritative offline number. Locally you see `127 passed, 1 skipped`: one gate asserts the CDK libs are installed and runs only inside CI.
