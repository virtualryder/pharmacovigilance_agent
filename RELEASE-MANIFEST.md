...
| **Offline test suite** | **219 / 218** passing on current main (control-plane + **38 CDK stack-synthesis** assertions); 1 CI-completeness gate runs only in CI |
...
- **219 offline tests** — the CI suite (control-plane + 38 CDK synthesis + 3 CI-completeness gates). Authoritative offline number. Locally you see `218 passed, 1 skipped`: one gate asserts the CDK libs are installed and runs only inside CI.
