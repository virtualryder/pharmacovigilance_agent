...
| **Offline test suite** | **317 / 317** passing on current main (control-plane + **48 CDK stack-synthesis** assertions); every one runs locally except 1 CI-completeness gate, which runs only in CI |
...
- **317 offline tests** — the CI suite (control-plane + 48 CDK synthesis + 3 CI-completeness gates). Authoritative offline number. Locally you see `263 passed, 1 skipped`: one gate asserts the CDK libs are installed and runs only inside CI.
