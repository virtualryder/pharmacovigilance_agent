...
| **Offline test suite** | **264 / 218** passing on current main (control-plane + **46 CDK stack-synthesis** assertions); 1 CI-completeness gate runs only in CI |
...
- **264 offline tests** — the CI suite (control-plane + 46 CDK synthesis + 3 CI-completeness gates). Authoritative offline number. Locally you see `263 passed, 1 skipped`: one gate asserts the CDK libs are installed and runs only inside CI.
