...
| **Pilot tag** | `v0.4.0-pilot-rc1` (RELEASE file) — **cut 2026-09-05, offline-gated on governed-core 1.10.1**; `v0.3.0-pilot-rc1` was cut 2026-09-03 after the live governed-core 1.9.0 multi-tenant gate (`evidence/AGENTCORE-111-GATE-2026-09-03.md`). Earlier validated tags: `v0.1.1-pilot-rc1` (live EP1, 2026-07-27), `v0.2.0-pilot-rc1` (live EP2, 2026-08-03) **`v0.4.0-pilot-rc1` (2026-09-05) supersedes `v0.3.0-pilot-rc1`**: the same tree with `lib/` relocked at the pinned **governed-core 1.10.1** (fault-semantics batch — durable-evidence gating, un-strandable approvals, authoritative Cedar context — plus deepened PII/PHI detection); **offline-gated** (all pass locally except one CI-only gate; 325 collected). The last LIVE gate on this pack ran on governed-core 1.9.0 (2026-09-03, env `pv-mt`, two tenants, real AgentCore Runtime); a live re-gate on 1.10.1 is the next milestone for this pack (third external review, REL-2). |
| **Governance core** | `governed-core` **1.11.1** pinned (`requirements-core.txt`, hash-pinned wheel; `lib/core.lock`; offline-gated). Last LIVE-gated core: **1.9.0** <!-- count-gate:historical --> — multi-tenant routing, one correlation set through every hop, kill switch on the AgentCore path, per-tenant token + USD budget. Live gates 2026-09-03 on `pv-mt` (2 tenants, real AgentCore Runtime): 111 gate 12/12 + 13/13 per tenant + strict canary; kill switch 29/29; budget 24/24; regression sweep 0 unexpected. Records: `evidence/AGENTCORE-*-2026-09-03.*` |
| **Offline test suite** | **325 / 325** passing on current main (control-plane + **48 CDK stack-synthesis** assertions); every one runs locally except 1 CI-completeness gate, which runs only in CI |
| **Deployment IaC** | AWS CDK, 7 stacks single-tenant / 6 + one data stack per tenant multi-tenant (`cdk/pv_stacks`, prefix `pv-`) — synthesizes to valid CloudFormation (in-suite `aws_cdk.assertions`) |
...
- **325 offline tests** — the CI suite (control-plane + 48 CDK synthesis + 3 CI-completeness gates). Authoritative offline number. Locally you see `263 passed, 1 skipped`: one gate asserts the CDK libs are installed and runs only inside CI.
