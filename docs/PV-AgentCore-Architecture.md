...
6. **Manifest + validate** — the whole agent is one manifest; deploy; end-to-end run (Cedar allow/deny, live openFDA background, masking, seriousness + reporting clock, real Bedrock narrative, tamper-evident audit) + negative tests + a red-team harness; teardown. Validated live on a clean AWS account twice — pv-val1 (2026-07-27) and pv-val2 (2026-07-28, a full SA-runbook re-walk that cut the current tag): `validate_deployment.py` PASS, the deterministic controller ran to the human sign-off gate, DuplicateHold held, and the **strict PHI canary reported 0 leaks**. Offline coverage is a **128-test suite** including **23 CDK stack-synthesis security assertions**. Evidence: [`../evidence/EP1-VALIDATION.md`](../evidence/EP1-VALIDATION.md). All evidence is author-produced on synthetic data; no independent audit or penetration test has been performed.
