...
const body = [
  ...
  P("Pharmacovigilance intake decides whether an adverse-event report is a serious ICSR, how fast it must reach the regulator, and how it is coded and narrated. When an adverse-event source arrives, a regulated workflow runs: intake the non-PHI decision fields, pull non-authoritative FAERS reference background, de-identify PHI, assess ICH E2B(R3) seriousness and the reporting clock (expedited 15-day vs. periodic vs. routine), draft a CIOMS/ICSR narrative, obtain a qualified person's review and sign-off, and commit the submission to the safety system of record."),
  ...
  table(["HIPAA safeguard", "How the accelerator addresses it", "Evidence / sponsor responsibility"], [
    ["De-identify / minimum necessary", "The mask_pii tool runs Amazon Comprehend DetectPiiEntities to remove PHI (name, DOB, address, identifiers, contact info) before drafting and before the audit — fail-closed: if masking cannot run, no assessment or draft is produced.", "Comprehend detection; masking is proven by a server-signed sanitized_ref bound to the content — a caller-supplied deidentified boolean is refused outright, whether true or false (P0-1)."],
    ...
    ["Audit every access and disclosure", "Every governed action writes a tamper-evident record capturing INTENT → COMMITTED with a SHA-256 hash of the action payload and a timestamp, chained to the prior entry by a server-read head and an atomic compare-and-swap so concurrent writers cannot fork the chain; duplicates are rejected. Note: the hash covers the action payload, not the clinical narrative being approved.", "The pv-audit ledger + WORM bucket; tests prove append-only, chain integrity and duplicate rejection."],
  ...
  table(["Part 11 / GxP area", "How the accelerator addresses it", "Status / sponsor responsibility"], [
    ...
    ["Separation of duties at sign-off (supports Part 11 §11.10(d))", "The submission is committed only by a different qualified person through a Step Functions waitForTaskToken gate, with a bound, single-use token. Identity is taken from a JWKS-verified token, never from the request body, and denials are written to the ledger.", "Implemented and live-validated. NOT yet implemented: §11.70 signature-to-record binding (the approval hash covers approval metadata, not the approved narrative), §11.50 signature manifestation (printed name / meaning of signature), and §11.200 two-component signing. See docs/PART-11-MAPPING.md — these are open gaps, not sponsor-owned items."],
  ...
  callout("Demonstrated on a live clean account", [["With Cedar in ENFORCE mode: a reviewer's request to self-approve is blocked as a separation-of-duties violation; a different qualified person's approval succeeds; the submission finalizes only after approval; and re-using the token is rejected. Validated on a clean AWS account (pv-val2, 2026-07-28) and covered by a 327-test offline suite including 48 CDK security assertions. Evidence: evidence/EP1-VALIDATION.md. This is author-produced evidence on synthetic data; no independent audit or penetration test has been performed."]], G.colors.MINT, "E9F5EF"),
  ...
  table(["The accelerator provides", "The sponsor is responsible for"], [
    ...
    ["Reproducible IaC + the 327-test offline suite (48 CDK security assertions)", "Medical review of narratives, causality methodology, and the CSV validation package"],
...
Packer.toBuffer(doc).then((b) => { require("fs").writeFileSync(require("path").join(__dirname, "..", "PV-AgentCore-Regulatory-Adherence.docx"), b); console.log("wrote regulatory"); });
