...
| T7 | **Double-reporting** — the same ICSR submitted twice | Two distinct controls, covering two distinct routes. (a) Duplicate *cases*: deterministic duplicate detection → the `duplicate` guard HOLDS at the terminal DuplicateHold state (no submission). (b) Duplicate *commits of the same case*: an exactly-once `FINAL#` conditional-put marker in the append-only audit table is the single commit gate — a retried Lambda, a replayed execution, or a second approval path finds the marker and returns the ORIGINAL submission, writing no second COMMITTED record. | `tests/test_workflow_guards.py::test_duplicate_holds`; `tests/test_exactly_once_finalize.py` (4 tests, verified to fail when the gate is disabled) |
