...
| **B6 · Load / replay** — concurrency + exactly-once replay storm | (design + 1 unit test) | ◑ **Partial** — the conditional-put FINAL# marker is covered by a single idempotency unit test against a fake DynamoDB; there is **no concurrency test, no replay-storm test and no load harness** in this repo | ☐ prod-scale live load AND a concurrency/replay-storm harness (both customer-side Gate-B exit) |
...
- Signed `sanitized_ref` masking proof; token boundary; deterministic guard set with DuplicateHold;
  ...
  ENFORCE with the PV tool/policy set — all asserted by the **109-test suite** (incl. 23 CDK assertions).
