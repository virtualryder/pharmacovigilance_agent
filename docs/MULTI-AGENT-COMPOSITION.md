...
| # | Item | Status |
|---|---|---|
| 1 | Account id in `governed-agent-platform` **history** | **CLOSED 2026-08-03.** `git filter-repo` purged both generated artifacts from all commits and redacted any residual occurrence; force-pushed. Verified across every blob (206 scanned, 0 hits), 9 commits preserved, mirror backup taken first. |
| 2 | Agents consume the package by **pinned reference** | **CLOSED 2026-08-03.** The core is published as a hash-pinned wheel from the public `virtualryder/governed-core` repo; all four agents install `governed-core 1.4.0` with `--require-hashes` and carry no copy. `tools/check_core_parity.py` now asserts pin + hash agreement and that no core module has been copied back. |
| 3 | Live re-validation of exactly-once in PV | **CLOSED 2026-08-03.** Proven live on `pv-val3`, including under real concurrency — 8 simultaneous finalizes produced 1 commit and 1 COMMITTED record. The `signoff_register` duplicate-submission control was proven in the same run. See [`../evidence/EP2-CONCURRENCY-LIVE.md`](../evidence/EP2-CONCURRENCY-LIVE.md). |
| 4 | Prod-scale load / sustained replay storm | **Open — customer-side.** The 8-way race is a correctness proof of the conditional put, not a throughput test. Sustained load remains a Gate-B exit item, along with an enterprise IdP round-trip and independent security testing. |
