...
| Immutable WORM audit (Part 11 / GxP evidence) | Build | Append-only DynamoDB + S3 Object Lock; Observability traces are for ops, not tamper-proof evidence |
...
- **Immutable WORM audit:** an append-only, tamper-evident record (append-only DynamoDB + S3 Object Lock) capturing `INTENT → COMMITTED` for the 21 CFR Part 11 / GxP evidence trail.
...
6. **Manifest + validate** — the whole agent is one manifest; deploy; end-to-end run (Cedar allow/deny, live openFDA background, masking, seriousness + reporting clock, real Bedrock narrative, immutable audit) + negative tests + a red-team harness; teardown. Proven live: **30/30 governance checks in ENFORCE** and **7/7 red-team ("governance holds under attack")**.
...
- **HIPAA** (safeguarding PHI in the adverse-event case) → fail-closed `mask_pii` + least-privilege Cedar + immutable audit + encryption.
