...
> 🤖 **AgentCore-native, live in ENFORCE.** Runs on **Amazon Bedrock AgentCore Gateway (MCP, CUSTOM_JWT) + a Cedar Policy engine in ENFORCE** (deny-by-default, forbid-wins), attached as IaC — live-validated and torn down (EP1 `pv-val1`, 2026-07-27; `pv-val1-gateway` AgentCore attachment = ENFORCE). Aegis adds what AgentCore does **not**: bound single-use SoD approval, fail-closed masking-before-audit, WORM tamper-evident audit, and regulator control mappings. AgentCore Policy is GA (2026-03-03); the reviewed `governed-core` engine stays as a defense-in-depth parity oracle + offline/outage fallback. Portfolio spec: `aegis-ai-governance-platform-aws/docs/AGENTCORE-INTEGRATION.md`.
...
> Evidence: `evidence/EP1-VALIDATION.md`; tag `v0.1.1-pilot-rc1` (then `v0.2.0-pilot-rc1` after EP2, `v0.3.0-pilot-rc1` after the 2026-09-03 1.9.0 gate). Suite: **228 offline tests**
> (control-plane + 44 CDK synthesis assertions). Remaining before real PHI: QPPV SME sign-off,
...
(**228 tests**, including 44 CDK stack-synthesis security assertions).
