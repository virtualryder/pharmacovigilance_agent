...
token boundary, deterministic Step Functions controller). **What's proven today: the 346-test offline
suite** (control-plane behavior + 48 CDK stack-synthesis security assertions) **plus two live
...
**346 offline tests (incl. 48 CDK synthesis)**, tags `v0.1.1-pilot-rc1` → `v0.4.0-pilot-rc1` (v0.4.0 offline-gated on 1.10.1; v0.3.0 is the last live-gated tag). Evidence:
...
24/24, regression sweep 0 unexpected). Next: a credentialed drug-safety (QPPV) SME sign-off, enterprise IdP round-trip, and
