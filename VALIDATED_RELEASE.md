...
| Governance core | `governed-core` **1.11.1**, pinned by URL + sha256 (`requirements-core.txt`, `--require-hashes`); `lib/core.lock` locked at 1.11.1 (`lib/verify_core.py` OK) — 1.9.0 <!-- count-gate:historical --> is the last core this pack was LIVE-gated on; `lib/runtime/` byte-identical with the benefits pack (shared runtime) |
